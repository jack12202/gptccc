...
test("嗨付 API Key 验证通过后持久化且管理响应不返回明文", async t => {
  ...
  const store = new HifupayCredentialStore({
    ...
    fetchImpl: async (url, options) => { requests++; assert.equal(String(url), "https://hifupay.example.test/api/hfp/login"); assert.deepEqual(JSON.parse(options.body), { apiKey, platform: "haifupaytop" }); return { ok: true, json: async () => ({ success: true, apiKey: "temporary-session-key" }) }; }
  });
