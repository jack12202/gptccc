...
function render(cards){return cards.map(card=>{const accounts=(card.plusUsers||[]).map(user=>esc((user.email||user.accountId||"未知账号")+(user.upgradeUntil?"（可升级至 "+date(user.upgradeUntil)+"）":""))).join("<br>")||"-";const pending=(card.inFlightOrders||[]).map(item=>'<br><small>待确认 '+esc(item.email||item.accountId||item.orderId)+' · '+esc(money(item.estimatedChargeUsd))+' · '+esc(date(item.reservedAt))+' <button class="secondary" data-action="release" data-id="'+esc(card.id)+'" data-order-id="'+esc(item.orderId)+'">释放</button></small>').join("");const pro=card.proReservation;const purpose=pro?'<strong>'+esc(pro.type||"Pro")+'</strong><br>'+esc(pro.account||"-")+(pro.renewalAt?'<br><small>预计续费 '+esc(day(pro.renewalAt))+'</small>':''):card.highBalanceProtected?'Plus 高余额保护<br><small>余额超过 '+esc(money(card.plusMaxBalance))+'</small>':'Plus 自动充值';const cls=["disabled","pro_protected"].includes(card.poolStatus)?"bad":["low_balance","reserved","upstream_unavailable","high_balance"].includes(card.poolStatus)?"warn":"";const enabledAction=card.enabled?'<button class="danger" data-action="disable" data-id="'+esc(card.id)+'">禁用</button>':'<button class="secondary" data-action="enable" data-id="'+esc(card.id)+'">启用</button>';const protectAction=card.proProtected?'<button class="secondary" data-action="protect-pro" data-id="'+esc(card.id)+'" data-account="'+esc(pro?.account||"")+'" data-type="'+esc(pro?.type||"20X Pro")+'" data-renewal="'+esc(pro?.renewalAt||"")+'">修改 Pro 信息</button><button class="secondary" data-action="release-pro" data-id="'+esc(card.id)+'">解除 Pro 保护</button>':'<button class="secondary" data-action="protect-pro" data-id="'+esc(card.id)+'">Pro 续费保护</button>';return '<tr><td>ID '+esc(card.id)+'<br>****'+esc(card.lastFour||"----")+'</td><td><input style="width:72px" type="number" min="0" data-setting="priority" data-id="'+esc(card.id)+'" value="'+esc(card.priority)+'"></td><td>'+esc(money(card.balance))+'<br><small>可用 '+esc(money(card.availableBalance))+'</small></td><td>'+esc(card.automaticPlusUsed)+' 人</td><td class="accounts">'+purpose+'</td><td><span class="badge '+cls+'">'+esc(labels[card.poolStatus]||card.poolStatus||"未知")+'</span></td><td class="accounts">'+accounts+pending+'</td><td><div class="actions">'+protectAction+enabledAction+'</div></td></tr>'}).join("")||'<tr><td colspan="8">暂无匹配卡片</td></tr>'}
