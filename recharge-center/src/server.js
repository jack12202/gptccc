...
<section class="panel"><h2>ZZS 网络出口</h2><p>当前使用 VPS 直连 IPv4。已保存的 MiyaIP 代理仍加密保留，需要时可手动启用；仅 ZZS API 请求受此设置影响。</p><div class="status-line" id="zzshuProxyState">读取中…</div><div class="actions"><button id="useZzshuDirect">使用直连 IPv4</button><button id="useZzshuProxy">使用已保存代理</button></div><div class="form" style="margin-top:10px"><input id="zzshuProxyUrl" type="password" autocomplete="new-password" placeholder="socks5://用户名:密码@节点主机:端口"><button id="saveZzshuProxy">验证并保存出口</button></div><div class="actions"><button id="checkZzshuProxyTcp">只检查节点端口</button><button id="checkZzshuEgressIp">查看服务器出口 IP</button></div><p class="muted">节点端口诊断只向本站后台发送主机和端口，不发送代理密码或 ZZS Key，也不保存出口。</p><div class="message" id="zzshuProxyMessage"></div></section>
...
async function save(channel,path){
  ...
  const button=document.getElementById(channel==="hifupay"?"saveHifupay":"saveZzshu"),replace=button.dataset.replace==="1";if(replace&&!confirm(channel==="zzshu"?"更换后，旧订单仍保存在本站；未完成的旧订单将停止自动查单，进入待核查。确认更换？":"新 API 验证成功后将替换当前 API。未完成订单存在时系统会拒绝更换。确认继续？"))return;
...
async function loadZzshuProxy(){try{const d=await api("/api/admin/zzshu/proxy");const state=document.getElementById("zzshuProxyState");state.textContent=d.usingProxy?"当前出口：已保存代理":"当前出口：VPS 直连 IPv4";state.className="status-line good";document.getElementById("saveZzshuProxy").disabled=!d.storageReady;document.getElementById("useZzshuProxy").disabled=!d.configured}catch(e){message("zzshuProxyMessage",e.message,true)}}
...
document.getElementById("useZzshuDirect").onclick=async()=>{try{await api("/api/admin/zzshu/proxy/mode",{method:"POST",body:JSON.stringify({mode:"direct_ipv4"})});message("zzshuProxyMessage","已切换为 VPS 直连 IPv4；代理资料仍加密保留。");await loadZzshuProxy()}catch(e){message("zzshuProxyMessage",e.message,true)}};
document.getElementById("useZzshuProxy").onclick=async()=>{try{await api("/api/admin/zzshu/proxy/mode",{method:"POST",body:JSON.stringify({mode:"proxy"})});message("zzshuProxyMessage","已启用保存的 ZZS 专用代理。");await loadZzshuProxy()}catch(e){message("zzshuProxyMessage",e.message,true)}};
...
document.getElementById("checkZzshuEgressIp").onclick=async()=>{const button=document.getElementById("checkZzshuEgressIp");button.disabled=true;message("zzshuProxyMessage","正在查询服务器出口 IP…");try{const d=await api("/api/admin/zzshu/proxy/egress-ip");message("zzshuProxyMessage","服务器出口 IP："+d.ip+"。可用于核对 MiyaIP 节点的服务器 IP 白名单。") }catch(e){message("zzshuProxyMessage",e.message,true)}finally{button.disabled=false}};
...
export const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname.startsWith("/api/admin/zzshu/")) {
      ...
      let body = {};
      if (req.method === "POST") {
        try { body = await readJsonBody(req, endpoint.startsWith("credential") || endpoint.startsWith("proxy") ? 4096 : Infinity); }
        catch { sendJson(res, 400, { success: false, message: "请求格式不正确。" }); return; }
      }
      ...
      try {
        ...
        if (req.method === "GET" && endpoint === "credential") data = zzshuCredentialStore.status();
        else if (req.method === "GET" && endpoint === "proxy/egress-ip") {
          const response = await fetch("https://www.cloudflare.com/cdn-cgi/trace", { signal: AbortSignal.timeout(5000) });
          const match = response.ok ? (await response.text()).match(/^ip=([^\r\n]+)$/m) : null;
          if (!match || !net.isIP(match[1])) { sendJson(res,502,{success:false,message:"暂时无法取得服务器出口 IP。"});return; }
          data = { ip: match[1] };
        }
        ...
        else if (req.method === "POST" && endpoint === "proxy/mode") {
          if (credentialRotationInProgress || activeRechargeConfirmations > 0) {
            sendJson(res,409,{success:false,message:"正在提交充值或更换凭据，请稍后切换出口。"});return;
          }
          const result = zzshuProxyStore.setMode(body.mode);
          if (!result.ok) { sendJson(res,result.status,{success:false,message:result.message});return; }
          data = zzshuProxyStore.status();
        }
          ...
          try {
            ...
            data = { configured: true, keyAccepted: true };
          } finally { credentialRotationInProgress = false; }
        }
          ...
          const [ipv4, ipv6] = await Promise.all([zzshuCredentialStore.probeFamily(4), zzshuCredentialStore.probeFamily(6)]);
          data = { networkReachable: result.upstreamHttpStatus === 401 && result.upstreamCode === 40107,
            ...
            upstreamRay: String(result.upstreamRay || "").slice(0, 80), ipv4, ipv6 };
        }
          ...
          credentialRotationInProgress = true;
          try {
            zzshuService.store.ensureQueryKey(zzshuCredentialStore.key());
            const result = await zzshuCredentialStore.verifyAndSave(body.apiKey, { replace: body.replace === true });
            if (!result.ok) { sendJson(res, result.status, { success: false, message: result.message }); return; }
            const oldOrdersAwaitingReview = zzshuService.store.ensureQueryKey(zzshuCredentialStore.key());
            data = { configured: true, points: result.points ?? null, channelEnabled: config.zzshuEnabled, oldOrdersAwaitingReview };
          } finally { credentialRotationInProgress = false; }
        }
